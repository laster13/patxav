  ...
  [http.services.streamfusion-svc]
    [http.services.streamfusion-svc.loadBalancer]
      ...
      [[http.services.streamfusion-svc.loadBalancer.servers]]
        url = "http://streamfusion:8080"
EOL
  echo "Fichier streamfusion_file créé."
else
...
PG_HBA_PATH=$(ls /etc/postgresql/*/main/pg_hba.conf 2>/dev/null | head -n 1)

if [[ -z "$PG_HBA_PATH" ]]; then
    echo "Fichier pg_hba.conf introuvable. Vérifiez l'installation de PostgreSQL."
    exit 1
fi

echo "Fichier pg_hba.conf trouvé : $PG_HBA_PATH"

# Modification du fichier pg_hba.conf pour configurer 'trust'
sudo sed -i "s/^local\s\+all\s\+postgres\s\+.*/local   all             postgres                                trust/" "$PG_HBA_PATH"
echo "Configuration de 'trust' pour l'utilisateur postgres effectuée."

# Redémarrage du service PostgreSQL
sudo systemctl restart postgresql
echo "Service PostgreSQL redémarré."

# Vérification de l'existence de la base de données
DB_EXISTS=$(psql -U postgres -tAc "SELECT 1 FROM pg_database WHERE datname='riven';")

if [[ "$DB_EXISTS" == "1" ]]; then
    echo "La base de données 'riven' existe. Suppression en cours..."
    psql -U postgres -c "DROP DATABASE riven;"
else
    echo "La base de données 'riven' n'existe pas."
fi

# Création de la base de données
echo "Création de la base de données 'riven'..."
psql -U postgres -c "CREATE DATABASE riven;"
echo "Base de données 'riven' créée avec succès."
...
if [ -d "$RIVEN_FRONTEND_DIR" ]; then
  ...
  sudo rm -rf "$RIVEN_FRONTEND_DIR"
fi

if [ -d "$RIVEN_DIR" ]; then
  echo "Suppression du dépôt riven existant..."
  sudo rm -rf "$RIVEN_DIR"
fi
