...
DB_EXISTS=$(PGPASSWORD="postgres" psql -U postgres -h localhost -tAc "SELECT 1 FROM pg_database WHERE datname='riven';")

if [[ "$DB_EXISTS" == "1" ]]; then
    echo "La base de données 'riven' existe. Suppression en cours..."
    PGPASSWORD="postgres" psql -U postgres -h localhost -c "DROP DATABASE riven;"
else
    echo "La base de données 'riven' n'existe pas. Création en cours..."
fi
