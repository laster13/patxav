...
DB_NAME="riven"
DB_USER="postgres"
DB_PASSWORD="postgres"  # Par défaut pour PostgreSQL
...
[http.routers]
  ...
  [http.routers.api-rtr]
      entryPoints = ["https"]
      rule = "Host(\`ssdv2.$domain\`) && PathPrefix(\`/scripts\`)"
      service = "api-svc"
...
if [ ! -f "$FILE" ]; then
    ...
    # Mise à jour ssdv2 avec python 3.11 pour eviter les conflits avec venv
    cd $HOME/seedbox-compose
    rm -rf venv
    python3.11 -m venv venv
    source venv/bin/activate
    python -m pip install --upgrade pip setuptools wheel
    pip install inquirer docker colorama ansible
    deactivate
fi
...
pm2 start "ORIGIN=https://ssdv2.$domain VITE_BACKEND_URL_HTTPS=https://ssdv2.$domain DIALECT=postgres DATABASE_URL=postgres://postgres:postgres@localhost:5432/riven node build" --name frontend --cwd "$RIVEN_FRONTEND_DIR"
